...
import org.example.data.Section;
...
public class OutputWriter implements Closeable {
    private static final String FILE_NAME = "C:\\Users\\kiari\\IdeaProjects\\income_statement_generator\\src\\main\\resources\\out\\Weekly Income Statement.xlsx";
...
//        XSSFColor.from()
    ...
    private static String path() {
        var folder = "C:\\Users\\kiari\\IdeaProjects\\income_statement_generator\\src\\main\\resources\\out";
        var format = new SimpleDateFormat("yyyyMMdd_hhmmss");
        var date = format.format(new Date());
        folder += "\\" + "Weekly Income Statement " + date + ".xlsx";
        return folder;
    }
    ...
    public void write(Map<Week, Set<Section>> data) throws IOException {
        ...
        sheet.setColumnWidth(0, width(maxCharsCol1));
        sheet.setColumnWidth(1, width(maxCharsCol2));
    ...
    private int width(AtomicInteger count) {
        return ((int) (count.get() + 4 * 1.14388) * 256);
    }
